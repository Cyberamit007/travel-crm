...
import { useLeadReport, usePerformanceReport } from '../../hooks/useReports';
...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'leads' | 'performance'>('leads');
  ...
  const handleExportLeads = () => {
    exportToCSV(`leads-report-${startDate}-${endDate}.csv`, [
      ...(leadSummary ? [{ metric: 'Total Leads', value: leadSummary.totalLeads }, { metric: 'Confirmed', value: leadSummary.confirmedLeads }, { metric: 'Lost', value: leadSummary.lostLeads }, { metric: 'Conversion Rate %', value: leadSummary.conversionRate }] : []),
      { separator: '---' },
      ...byStatus.map((r: any) => ({ Status: r.name, Count: r.count })),
    ]);
  };

  const handleExportPerformance = () => {
    exportToCSV(`performance-report-${startDate}-${endDate}.csv`, employees.map((e: any) => ({
      Employee: e.name,
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          onClick={activeTab === 'leads' ? handleExportLeads : handleExportPerformance}
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit">
        {([['leads', 'Lead Analytics'], ['performance', 'Employee Performance']] as const).map(([key, label]) => (
          <button
            key={key}
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div className="card p-5">
                  ...
                  {byStatus.length === 0 ? (
                    <p className="text-center text-slate-400 py-8 text-sm">No data</p>
                  ) : (
                ...
                <div className="card p-5">
                  ...
                  {bySource.length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <PieChart>
                        <Pie data={bySource} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
          ...
          {perfLoading ? (
            ...
            <>
              ...
              {topCampaigns.length > 0 && (
                <div className="card p-5">
                  <h3 className="font-semibold text-slate-800 mb-4 text-sm">Top Campaigns</h3>
