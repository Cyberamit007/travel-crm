...
export const getLeadJourney = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const stageDone: Record<(typeof STAGE_ORDER)[number], { done: boolean; at: Date | null }> = {
    ...
    const stages = STAGE_ORDER.map((key) => ({
      ...
      done: stageDone[key].done,
      at: stageDone[key].at,
    }));

    const completedCount = stages.filter((s) => s.done).length;
    // Current stage = last completed one (stages are meant to be monotonic, but
    // guard against gaps by walking from the end).
    let currentIndex = -1;
    for (let i = stages.length - 1; i >= 0; i--) { if (stages[i].done) { currentIndex = i; break; } }

    res.json({
      success: true,
      data: { stages, completedCount, totalCount: stages.length, currentStage: currentIndex >= 0 ? stages[currentIndex].key : null },
    });
  } catch (e) {
